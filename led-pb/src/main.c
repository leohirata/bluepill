...
Configurar a GPIO dos pinos do botão e do LED, de modo que o pino do botão seja de saída (output) e do LED de entrada (input).

Para o funcionamento da GPIO, deve-se habilitar seus respectivos clocks. 

Use o datasheet e escolha qual porta e pino da GPIO deseja usar. Também use o reference manual para saber qual registrador é
...
int main(void)
{
  ...
  while(1)
  {
    ...
    {
      GPIO_WriteBit(GPIOA, GPIO_Pin_5, GPIO_ReadOutputDataBit(GPIOA, GPIO_Pin_5)^1);
